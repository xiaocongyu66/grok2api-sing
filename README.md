...
- **多账号池**：basic / super / heavy 三级池，自动负载均衡、配额门控、优先级、并发限制、会话粘滞、冷却与故障切换
- **Console 免费模型**：`console.x.ai` SSO Token 接入，`*-console` / effort 别名零成本对话；本地配额延迟轮换（20 次 / 60 分钟窗口，剩余 ≤12 时启动恢复计时）
- **账号接入**：Device OAuth、OAuth JSON、SSO JSON、逐行 SSO Token
- **浏览器指纹**：Chrome TLS / HTTP2 头序与客户端提示；Cloudflare Cookie（`cf_clearance` 等）会话复用
- **反 bot 签名**：默认进程内纯 Go 生成 `x-statsig-id`（无需远程签名服务或浏览器）；可选 manual / 自建 url 签名
- **媒体生成**：文生图、图像编辑、文生视频、图生视频；本地磁盘或 Cloudflare R2 缓存；`/v1/media/images/{id}` 代理链接
- **基础设施**：SQLite/PostgreSQL、Memory/Redis、HTTP 与 SOCKS 代理池
- **安全边界**：AES-256-GCM 凭据加密、客户端密钥哈希、日志脱敏、SSRF 与传输上限
- **管理后台**：Token / 账号 CRUD、配置热更新、批量操作、Dashboard、模型、客户端密钥、请求审计
...
Grok Console 内置模型（`console.x.ai` SSO，不消耗 grok.com 付费额度）：

| 模型 | 能力 | 兼容别名 |
| :-- | :-- | :-- |
| `grok-4.3` | Responses / Chat / Messages | `grok-4.3-console`、`grok-4.3-low/medium/high` |
| `grok-4.20-0309` | Responses / Chat / Messages | `grok-4.20-0309-console` |
| `grok-4.20-0309-reasoning` | Responses / Chat / Messages | `grok-4.20-0309-reasoning-console` |
| `grok-4.20-0309-non-reasoning` | Responses / Chat / Messages | `grok-4.20-0309-non-reasoning-console` |
| `grok-4.20-multi-agent-0309` | Responses / Chat / Messages | `grok-4.20-multi-agent-console`、`*-low/medium/high/xhigh` |
| `grok-build-0.1` | Responses / Chat / Messages | `grok-build-console` |

**Console 配额轮换**：本地窗口默认 20 次 / 60 分钟。扣减时优先选择剩余额度更高的账号；`remaining` 降到 12 及以下才启动恢复计时器，到期后自动重置，避免同批账号被反复选中。
...
# 或 X-API-Key: g2a_xxx_xxx
# 或配置 auth.apiKeyHeaders 后使用自定义头，例如：
# congyu_15fc: g2a_xxx_xxx
```
